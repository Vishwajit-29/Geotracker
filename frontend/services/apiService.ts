...
// In development (Vite dev server on port 5173), connect directly to backend:8080
// In all other cases (Nginx on port 80/3000 in Docker), use relative /api proxy path
const getApiBaseUrl = (): string => {
  ...
  // Vite dev server runs on 5173 by default — connect directly to backend
  if (port === "5173") {
    return `${protocol}//${hostname}:8080/api`;
  }
  // Nginx (Docker local on :3000 or production on :80) — use reverse proxy
  return "/api";
};
...
export const getMySchedule = async (): Promise<import('../types').WorkSchedule | null> => {
  try {
    const response = await fetchWithAuth('/schedule/me');
    if (response.status === 204) return null;
    const data = await response.json();
    return data as import('../types').WorkSchedule;
  } catch { return null; }
};

export const getScheduleForUser = async (userId: number): Promise<import('../types').WorkSchedule | null> => {
  try {
    const response = await fetchWithAuth(`/schedule/${userId}`);
    if (response.status === 204) return null;
    const data = await response.json();
    return data as import('../types').WorkSchedule;
  } catch { return null; }
...
export const getAllSchedules = async (): Promise<import('../types').WorkSchedule[]> => {
...
export const setSchedule = async (
  userId: number,
  schedule: { workStartTime: string; workEndTime: string; workDays: string[]; active: boolean },
): Promise<import('../types').WorkSchedule> => {
  const response = await fetchWithAuth(`/schedule/${userId}`, {
    method: 'POST',
...
export const deleteSchedule = async (userId: number): Promise<void> => {
  await fetchWithAuth(`/schedule/${userId}`, { method: 'DELETE' });
};

// ==================== Auto Attendance ====================

export const autoCheckIn = async (location: import('../types').Location): Promise<AttendanceRecord> => {
  const response = await fetchWithAuth('/attendance/auto-checkin', {
    method: 'POST',
    body: JSON.stringify(location),
  });
  const record = await response.json();
...
export const autoCheckOut = async (): Promise<AttendanceRecord> => {
  const response = await fetchWithAuth('/attendance/auto-checkout', { method: 'POST' });
  const record = await response.json();
  return {
    id: record.id,
    userId: record.userId,
